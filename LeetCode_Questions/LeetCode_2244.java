// LeetCode question LeetCode 2244
...
public class LeetCode_2244 {
    ...
    public int minimumRounds(int[] tasks) {
        ...
        for (int i = 0; i < tasks.length; i++) {
            ...
            count++;
            // If we have reached the last task or the current task is different from the
            // next task
            if (i == tasks.length - 1 || tasks[i] != tasks[i + 1]) {
                ...
                // Add the number of full groups of 3 we can create from the tasks with this
                // value
                result += count / 3;
