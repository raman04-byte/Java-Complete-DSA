public class LeetCode_918 {
    ...
    public int maxSubarraySumCircular(int[] array) {
        ...
        // variable to keep track of the minimum sum subarray using kadane's algorithm
        // on the negated array
        int min = kadane(array);
        // variable to keep track of the maximum sum subarray that can be formed by
        // wrapping around the array
        int max2 = acc + min;
        // if the maximum sum subarray that can be formed by wrapping around the array
        // is zero, return the maximum sum subarray using kadane's algorithm
        if (max2 == 0)
            return max1;
    ...
    public int kadane(int[] array) {
        ...
        for (int i = 1; i < array.length; i++) {
            ...
            // by taking the maximum between the current element and the sum of the current
            // element and the maximum sum subarray ending at the previous index
            maxSum = Math.max(maxSum + array[i], array[i]);
            // update the overall maximum sum subarray by taking the maximum between the
            // current maximum sum subarray ending at current index and the overall maximum
            // sum subarray
